def tsp(cities, paths, dist):
    ...
    while path_idx < len(all_paths):
        ...
        city_idx = 0
        total_distance = 0
        while city_idx < (len(path) - 1):
            total_distance += paths[path[city_idx]][path[city_idx + 1]]
            city_idx += 1
        # print("total_distance", total_distance=-)
        if total_distance < dist:
            return True
        path_idx += 1
        if total_distance != prev_dist:
            print("total_distance+++++++++++", total_distance, path, path_idx)
